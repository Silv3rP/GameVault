...
**Search Functionality** — On form submission, the search term is sent to the CheapShark API using `async/await` and `fetch()`. Results are displayed as cards showing the game title, sale price, normal price, thumbnail, which acts as a link to the deal, and Steam rating info. The rating display adapts depending on what data is returned — if no rating exists the card shows "No rating". Results are limited to 12 and only appear after the user submits, not on every keystroke. The search result cards use a `transform: scale(1.05)` on hover to make them stand out. After the first search is submitted on a fresh page load, a hint reading "Press Esc or Delete to clear" appears above the results. This fades out automatically after 3 seconds using the same `setTimeout` and `message-fade-out` class approach used for the form messages.
