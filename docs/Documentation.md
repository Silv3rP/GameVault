...
Expanding the carousel from four slides to twelve introduced a problem with the navigation dots. Originally each slide had its own dot, but with twelve slides this would have created twelve dots which looked cluttered and messy. The solution was to keep four dots, one per genre, and use `Math.floor(currentIndex / 3)` to calculate which dot should be active based on the current slide index. Clicking a dot also needed updating. Instead of jumping to the dot's index directly, it now jumps to `index * 3` to land on the first slide of that genre group. A further issue was that the original loop used `dots[i]` alongside the slide index, so when `i` reached 4 or higher `dots[i]` returned `undefined` and threw an error, stopping the function before the active class could be applied to the current slide. This was fixed by separating the dot and slide loops, with slides looping using their own index while dots are cleared using `forEach`. The auto-rotation interval was also reduced from five seconds to three seconds to keep the rotation feeling responsive across all twelve slides.
...
**Carousel Navigation** — A `currentIndex` variable tracks the active slide. Clicking next or previous buttons updates the index and calls `updateCarousel()` to show the correct slide and highlight the matching dot. The carousel auto-advances every three seconds using `setInterval()`, and pauses when the user hovers over it using `clearInterval()`. The carousel features all twelve curated games across four genres, with three slides per genre. Rather than displaying twelve navigation dots which would clutter the UI, four dots are used — one per genre. `Math.floor(currentIndex / 3)` calculates which dot to highlight based on the current slide, so the correct genre dot stays active across all three of its slides. Clicking a dot jumps to the first slide of that genre using `index * 3`. The interval was reduced from five seconds to three seconds to keep the rotation feeling responsive across all twelve slides.
